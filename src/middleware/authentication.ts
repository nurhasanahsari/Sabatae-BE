...
class AuthGuard {
  ...
  public checkTokenTeacher = async (req: any, res: Response, next: NextFunction) => {
    try {
      if (!req.headers.authorization) {
        return response(res, 401, 'Unauthorized Access');
      }
      ...
      const check = await this.verifyToken(token, config.server.access_secret, false, true);
      req.users = check.data;
      if (!['01', '02', '03'].includes(check.data.role)) {
        return response(res, 401, 'Unauthorized Access');
      }
      next();
    } catch (error: any) {
  ...
  public checkTokenStudent = async (req: any, res: Response, next: NextFunction) => {
    try {
      if (!req.headers.authorization) {
        return response(res, 401, 'Unauthorized Access');
      }
      ...
      const check = await this.verifyToken(token, config.server.access_secret, false, true);
      req.users = check.data;
      if (!['01', '02', '03', '04'].includes(check.data.role)) {
        return response(res, 401, 'Unauthorized Access');
      }
      next();
    } catch (error: any) {
